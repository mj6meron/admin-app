...
exports.deleteUser = async (req, res) => {
  ...
  if (error) {
    signale.fatal(
      'user remove validation failed - u need to pass in user_id:"asdfaf"'
    );
    return res.status(400).json({ error: error.details[0].message }); // The message is form the joi object in validation.js
  }
 ...
 //    Product.foreach(p=>p.remove())
  //  await product.remove();
    ...
    return res
      .status(400)
      .json({ message: `User of id ${id} succefully Removed!` });
  } catch (error) {
    signale.fatal("Something went wrong deleting User");
    return res
      .status(404)
      .json({ errorMessage: "Something went wrong deleting User" });
  }
};
